...
namespace EquipmentReservations.Domain.Services
{
  public class EquipmentService : IEquipmentService
  {
    ...
    public EquipmentService(IUnitOfWork unitOfWork, IEquipmentRepository equipmentRepository,
     ICategoryService categoryService, IGuidProvider guidProvider)
    {
    ...
    public async Task<Equipment> AddAsync(Equipment equipment)
    {
      ...
      var category = await categoryService.GetByIdAsync(equipment.CategoryId);

      if (category == null)
        throw new MissingDataException($"Nie znaleziono kategorii o id: {equipment.CategoryId}");
      equipment.Category = category;

      equipment.Id = guidProvider.GenerateGuid();
      var added = await equipmentRepository.AddAsync(equipment);
      await unitOfWork.SaveChangesAsync();
      var result = await equipmentRepository.GetByIDAsync(added.Id);
      return result;
    }
    public async Task<Equipment> UpdateAsync(Equipment equipment)
    {
      ...
      try
      {
        ...
        await unitOfWork.SaveChangesAsync();
        var result = await equipmentRepository.GetByIDAsync(updated.Id);
        return result;
      }
